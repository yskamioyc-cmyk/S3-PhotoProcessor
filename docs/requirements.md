...
- 「保存テーブル」には、アップロードされた画像の加工・変更履歴を主に格納し、バージョン管理を行う。
- 「中間テーブル」は、「多対多」の関係になる画像と画像検索用タグの間を、タグマスタで管理するタグナンバーを仲介して結びつける役割を果たす。
...
| 管理番号 | 項目 | 内容（目標値） |
| :-- | :-- | :-- |
| NF-01 | セキュリティ | パスワードはBcrypt等を用いハッシュ化して保存する。 |
| NF-02 | セキュリティ | ログインセッションの有効期限は最終操作から２時間とする。 |
| NF-03 | 性能 | 10MBまでの画像を5秒以内にアップロード完了すること。 |
| NF-04 | 運用 | GitHub Actionsを利用し、メインブランチへのマージ時に自動テストを実行する。 |
| NF-05 | 信頼性 | AWS S3を利用し、画像データの耐久性を確保する。 |

# 5. システム制約・前提条件
- **利用ブラウザ**: Google Chrome最新版を推奨ブラウザとする。
- **画像形式**: 画像ファイルの形式はJPEG、PNG、WebPとし、1枚当たり10MBをサイズ上限とする。
- **スマホ対応**: 本プロジェクトではPCブラウザでの利用を優先とし、レスポンシブ対応は最低限とする。